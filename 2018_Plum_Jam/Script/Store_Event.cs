...
public class Store_Event : MonoBehaviour {
    ...
    {
        int money = GameObject.Find("Plum").GetComponent<Status>().Fund;
        ...
        if (money >= value && money>0 && item[index]==0)
        {
            ...
            audio.volume = 0.5f;
            audio.Play();
            GameObject.Find("Plum").GetComponent<Status>().Get_Fund_InOut(-int.Parse(GetComponentInChildren<Text>().text));
