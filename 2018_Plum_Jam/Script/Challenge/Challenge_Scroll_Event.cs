    ...
    public void Select()
    {
        if (challenge_Scroll.speed > 0.01f)
        {
            ...
            if (challenge_Scroll.slider.value > 50 - range && challenge_Scroll.slider.value <= 50 + range) // Win
            {
                ...
                for (int i = 1; i < 5; i++)
                {
                    if (transform.parent.Find("Text").GetComponent<Text>().text.Contains(gameInfo_stat[i].name))
                    {
                        ...
                        GameObject.Find("Plum").GetComponent<Status>().Get_Member_Status_Change_By_Addition(4f, -2f, avg);
                        break;
                    }
                }
            }
            else // Lose
            {
                ...
                GameObject.Find("Plum").GetComponent<Status>().Get_Member_Status_Change_By_Addition(-5f, -5f, -3f);
            }
